...
export function AddExpenseModal({ open, onClose, expense }: Props) {
  ...
  useEffect(() => {
    if (expense) {
      ...
      setTitle(""); setAmount(""); setCurrency(user?.currency ?? "NGN")
      setCategoryId(""); setPaymentMethod("CASH"); setDate(formatDateInput(new Date())); setNote("")
    }
    setErrors({})
  }, [expense, open, user])
            ...
            <div className="flex gap-2 flex-wrap">
              {Object.entries(PAYMENT_METHOD_LABELS).map(([k, v]) => (
                <button key={k} type="button" onClick={() => setPaymentMethod(k)}
                  className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium border transition-colors ${paymentMethod === k ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-accent"}`}>
                  {v}
                </button>
              ))}
            </div>
